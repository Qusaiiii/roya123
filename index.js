...
if(member.voiceChannelID === '478191935153111051') {
...
deny: ['CONNECT']
    ...
    const parent = member.guild.channels.get('478191935153111051').parentID
...
message.guild.members.get(user.id).setVoiceChannel('478191652406689792'); // المكان الي راح ينحطوله بعد ما يصير لهم lock
}  
} catch (error) {
console.log(error)
}
...
client.on("ready", () => {
    var guild;
    while (!guild)
        guild = client.guilds.find("name", "👑◾{ Hano Army }◾👑")
...
delete warn[message.author.id];
	...
	const embed500 = new Discord.RichEmbed()
     .setTitle(`mark:  | There is someone trying to spam `)
...
if(message.content.split(' ')[0] == '-bc') {
    if (!args[1]) {
message.channel.send("**| Hano. | Usage: -bc <message>**");
return;
}
...
const cuttweet = [
 ...
 client.on('message', message => {
  ...
  .setColor('RANDOM')
   .setThumbnail(message.author.avatarURL) 
 .addField('Hano. v0.1 | لعبه كت تويت' ,
...
client.on('message', message => { 
  var whitelisted = "292357771943477252"// ايديك
...
client.on('message', ( message ) => {
  ...
  if(message.channel.id !== '476045689521045524') return;
  ...
  if (message.attachments.size <= 0) {
    message.delete();
    message.channel.send(`${message.author}, **| هذا الشات مخصص فقط للصور**`)
    .then(msg => {
      setTimeout(() => {
        msg.delete();
      }, 5000)
...
client.on('message', async message => {
  ...
  let warnchannel = message.guild.channels.find(`name`, "hano↭log");
...
client.login('NDc3ODE1NjI5Njg0OTMyNjI5.DlBoQA.W5jRixgjcBv2byvQObpkndJ4j-M');
